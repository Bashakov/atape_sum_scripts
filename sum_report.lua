...
local function rep_temperature(marks, dest, table_desc, left_rail)
...
--		row[8] = sprintf("$frame(%d,%d)", vdCh, frcoord-1024)	
--		row[9] = sprintf("$frame(%d,%d)", vdCh, frcoord)	
--		row[10] = sprintf("$frame(%d,%d)", vdCh, frcoord+1024)	
	...
	local marks_process_fn = {
		...
		["{DC2B75B8-EEEA-403C-8C7C-212DBBCF23C6}"] = false, -- M_SPALA
	}
	local rep_rail = (tonumber(Passport.FIRST_LEFT) == left_rail) and 1 or 2
--	dest:SetCellText(4, 2, (left_rail==1) and "Левый" or "Правый")
	
	local key_fn = function(mark)	-- сортируем отметки по координате
		local prop = mark:prop()
		return {prop.RailMask, prop.SysCoord}
	end
	local filter_fn = function(mark) -- оставляем отметки магнитного и определенного рельса
		local prop = mark:prop()
		return prop.RailMask == rep_rail and marks_process_fn[prop.Guid] ~= nil
	end
	...
	for mark in SortKey( marks:range(), key_fn, filter_fn) do 
		local prop, ext, report = mark:prop(), mark:ext(), mark:report()
		local sys_coord = prop.SysCoord + prop.Len / 2 + offsetMagn
...
--		end
		...
		local temperature = Driver:GetTemperature(prop.RailMask==1 and 0 or 1, prop.SysCoord)
		temperature = temperature and temperature.target or ""
		
		local res_row = { km, m, make_link(0, report.name, prop.ID), "", 
			temperature, "", make_link(sys_coord-1000, sys_coord), report.human_rail}
...
local function rep_temperature_l(marks, dest, table_desc)
	return rep_temperature(marks, dest, table_desc, 1)
end

local function rep_temperature_r(marks, dest, table_desc)
	return rep_temperature(marks, dest, table_desc, 0)
end
...
--	{name="test2", 							fn=test_rep2,			filename="ProcessSum.xls",	sheetname="test"},
	{name="Температуры (левая нитка)",		fn=rep_temperature_l,	filename="ProcessSum.xls",	sheetname="Температуры"},
	{name="Температуры (правая нитка)",		fn=rep_temperature_r,	filename="ProcessSum.xls",	sheetname="Температуры"},
}
